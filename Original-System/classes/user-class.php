...
class User{
	...
	public function list_user_search($keyword){
		...
		$q = $this->conn->prepare('SELECT * FROM tb_User WHERE FullName LIKE ?');
	...
	public function upload_user_img($UserImage,$UserId){

		$sql = "UPDATE tb_user SET UserImage=:UserImage WHERE UserId=:UserId";

		$q = $this->conn->prepare($sql);
		$q->execute(array(':UserImage'=>$UserImage,':UserId'=>$UserId));
		return true;
	}
